...
# 26年最新版雛形: シート名は「報告書」。対象従業員(12)は雛形では貼り付けデータから
# XLOOKUP するが、画面で入力された場合はその値で上書きする。
# 決算書BS(38)は数式で自動化されたため入力行から除外。地区平均は店コード(D4)から自動。
EXCEL_SHEET = "報告書"
# テンプレートの「入力セル」がある行(数式行・自動計算行は書き込まない)
# 12(対象従業員)は雛形では貼り付けデータからの XLOOKUP だが、画面で人数を直した場合は
# その値を優先する(空欄なら書き込まないので、雛形の XLOOKUP がそのまま残る)。
EXCEL_INPUT_ROWS = {9, 12, 15, 17, 19, 21, 23, 25, 27, 29, 31, 33, 35, 37}
